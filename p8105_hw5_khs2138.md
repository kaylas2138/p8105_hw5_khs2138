...
sim_results_df = 
  ...
  mutate(p_value = map(data, ~t.test(.x, mu = 0, conf.level = 1 - alpha)))

sim_results_df
```

    ## # A tibble: 5,000 × 5
    ##    sample_size  iter data       mu_result p_value
    ##          <dbl> <int> <list>         <dbl> <list> 
    ##  1          30     1 <dbl [30]>     0.412 <htest>
    ##  2          30     2 <dbl [30]>     0.664 <htest>
    ##  3          30     3 <dbl [30]>     0.551 <htest>
    ##  4          30     4 <dbl [30]>     0.567 <htest>
    ##  5          30     5 <dbl [30]>    -1.65  <htest>
    ##  6          30     6 <dbl [30]>     1.19  <htest>
    ##  7          30     7 <dbl [30]>     0.334 <htest>
    ##  8          30     8 <dbl [30]>    -1.19  <htest>
    ##  9          30     9 <dbl [30]>     0.122 <htest>
    ## 10          30    10 <dbl [30]>     0.684 <htest>
    ## # ℹ 4,990 more rows
...
- Make a plot showing the average estimate of $\hat{\mu}$ on the y axis
  ...
  value of $\mu$ on the x axis. Is the sample average of $\hat{\mu}$
  across tests for which the null is rejected approximately equal to the
  true value of $\mu$? Why or why not?
